...
def create_claim(current_user):
    ...
    existing = Claim.query.filter_by(item_id=item_id, claimant_id=current_user.id).first()
    if existing:
...
def get_claims_for_item(current_user, item_id):
    ...
    my_claim = Claim.query.filter_by(item_id=item_id, claimant_id=current_user.id).first()
    if my_claim:
         return jsonify([{
            "id": my_claim.id,
            ...
            "meeting_time": my_claim.meeting_time.isoformat() if my_claim.meeting_time else None,
...
def respond_to_claim(current_user, claim_id):
    ...
    if action == 'reject':
        ...
        if not meeting_loc or not meeting_time_str:
            return jsonify({"error": "Meeting location and time required for acceptance"}), 400
            
        claim.meeting_location = meeting_loc
        try:
             # handle simple ISO format or standard JS toISOString
             claim.meeting_time = datetime.fromisoformat(meeting_time_str.replace('Z', '+00:00'))
        except:
        ...
        return jsonify({"message": "Claim accepted. Code generated.", "qr_token": code}), 200
...
def verify_claim_qr(current_user):
    ...
    try:
        ...
        if item.user_id != current_user.id:
            return jsonify({"error": "Only the original Finder (Item Uploader) can verify this claim."}), 403
...
def mark_notification_read(current_user):
    ...
    try:
        read_list = json.loads(current_user.read_notifications) if current_user.read_notifications else []
    except:
        read_list = []
...
def get_notifications(current_user):
    ...
    incoming_claims = Claim.query.filter(Claim.item_id.in_(my_item_ids)).order_by(Claim.timestamp.desc()).all()
