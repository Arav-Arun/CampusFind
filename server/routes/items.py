...
def create_item():
    ...
    try:
        ...
        final_category = data.get('category') if data.get('category') else analysis.get('category')
        ...
        new_item = Item(
            ...
            description=analysis.get('description', data.get('description', 'No description')),
        ...
        if new_item.type == 'found':
            try:
                ...
                vq = generate_verification_question(new_item.description, distinctive_features)
    ...
    try:
        ...
        for item in items:
            ...
            result.append({
                ...
                "color": item.color,
                "brand": item.brand,
                "distinctive_features": json.loads(item.distinctive_features) if item.distinctive_features else [],
                "status": item.status
            })
...
def reanalyze_item(id):
    ...
    try:
        ...
        # Get full path
        img_path = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'uploads', os.path.basename(item.image_url))
        ...
        if len(matches) == 0:
            ...
            for cand in candidates:
                ...
                if source_item.color and cand.color:
                    if source_item.color.lower() in cand.color.lower() or cand.color.lower() in source_item.color.lower():
                        score += 30
                ...
                if score >= 30: # Threshold
                    fallback_matches.append({
                        "item": {
                            ...
                            "image_url": cand.image_data if cand.image_data else (f"/uploads/{os.path.basename(cand.image_url)}" if cand.image_url else None)
                        },
                        "confidence": score,
            ...
            # Sort by score
            matches = sorted(fallback_matches, key=lambda x: x['confidence'], reverse=True)[:5]
