      ...
      <div className="bg-surface rounded-xl overflow-hidden border border-white/5 hover:border-accent/30 transition-all duration-300 shadow-lg hover:shadow-accent/10 group cursor-pointer hover:scale-[1.02] active:scale-[0.98]">
        ...
        <div className="p-4">
          ...
          <div className="mt-4 pt-4 border-t border-white/5 flex flex-wrap gap-2">
            {item.category && (
              <span className="px-2 py-1 rounded-md bg-secondary/30 text-xs text-muted flex items-center gap-1">
                <Tag size={10} /> {item.category}
              </span>
            )}
            {item.color && (
              <span className="px-2 py-1 rounded-md bg-secondary/30 text-xs text-muted">
                {item.color}
              </span>
            )}
          </div>
